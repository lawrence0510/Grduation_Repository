...
class GetRateFromAnswers(Resource):
    ...
    def post(self):
        ...
        rate_standard = (
            "評分項目定義\n"
            "1. 語意（Semantics）： 語意是指語言單位（如單詞、短語、句子）的意義及其理解方式。它關注的是語言中詞彙和句子的內在意涵及其與現實世界之間的關聯。例如，語意研究會考慮「貓」這個詞代表一種動物，「天氣很好」這句話所傳達的具體含義。"
            "\n"
            "2. 語用（Pragmatics）：是指語言在具體情境中的使用方式及其所傳達的隱含意義。它考慮的是說話者和聽話者之間的互動、語境對語義的影響、以及語言的實際用途。例如，「你能關門嗎？」在語法上是一個問句，但在語用上，它實際上是一個請求。語用的研究涉及理解說話者的意圖、背景知識和交流情境。"
            "\n"
            "3. 語法（Syntax）： 語法是指語言中單詞、短語和句子如何組合成符合規則的結構。它關注的是語言的形式和結構，如句子的主詞、動詞和賓語的排列順序。語法規範了單詞如何組合成句子，以使其具有正確的結構和清晰的邏輯。例如，在中文中，語法規則要求「我吃了蘋果」是正確的句子，而「吃我了蘋果」則不符合語法。"
            "\n"
            "另外，請先檢查這段話有沒有錯字，若有錯字的話要在總評講出來，並且根據數量扣語用的分數"
        )
            ...
            try:
                ...
                if article:
                    ...
                    prompt_message = (
                        ...
                        f"請你根據以下三個評分面向：語意、語用、語法，去對我的回答做評分："
                        f"三個各自最低1分，最高5分，並針對你做出的這三個評分給出理由，"
                        f"最後再給一個總評。並以JSON格式回傳，格式如下："
                        f'{{"總評": 詳述使用者回答狀況並提出優點與缺點, "語意得分": 1~5, "語意評分理由": 理由, '
