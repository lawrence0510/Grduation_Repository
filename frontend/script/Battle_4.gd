...
var current_score_1 = GlobalVar.player_score  # 玩家計分區塊的當前分數
var target_score_1 = GlobalVar.player_score  # 玩家計分區塊的目標分數（用來平滑過渡）
var current_score_2 = GlobalVar.opponent_score  # 對手計分區塊的當前分數
var target_score_2 = GlobalVar.opponent_score  # 對手計分區塊的目標分數（用來平滑過渡）
var base_score_per_question = 120  # 每題基本分數
var question_start_time = 0  # 記錄答題開始時間

# 第一組題目
var question_content = GlobalVar.battle_question["short_question4"]
var options = [GlobalVar.battle_question["shortquestion4_option1"], GlobalVar.battle_question["shortquestion4_option2"], GlobalVar.battle_question["shortquestion4_option3"], GlobalVar.battle_question["shortquestion4_option4"]]
var correct_answer = GlobalVar.battle_question["shortquestion4_answer"]
...
func _on_button_pressed(button_path: String):
	...
	update_compete_request(4, str(selected_answer))
	# 禁用其他按鈕
...
func _on_HTTPRequest2_request_completed(result, response_code, headers, body):
	if response_code == 200:  # 成功接收回應
		...
		if GlobalVar.user_id == json_data["user1_id"]:
			opponent_answer = json_data["user2_question4"]
			opponent_score = json_data["user2_score"]
		elif GlobalVar.user_id == json_data["user2_id"]:
			opponent_answer = json_data["user1_question4"]
